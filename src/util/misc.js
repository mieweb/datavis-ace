...
Timing.prototype.start = function (what) {
	...
	self.logDebug(self.makeLogTag() + ' Received <START> event for [' + subject + ' ] ' + event);
...
Timing.prototype.stop = function (what) {
	...
	self.logDebug(self.makeLogTag() + ' Received <STOP> event for [' + subject + ' ] ' + event);

	if (getProp(self.data, subject, event, 'start') === undefined) {
		self.logWarn(self.makeLogTag() + ' Received <STOP> event for [' + subject + ' : ' + event + '] with no <START> event');
		return;
	}
...
Timing.prototype.dump = function (subject) {
	...
	var f = function (sub) {
		...
		_.each(self.events[sub], function (evt) {
			...
			self.logInfo(self.makeLogTag() + ' ' + sub + ' : ' + evt + ' >> ' + (end - start) + 'ms');
		});
	};
