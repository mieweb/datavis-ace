...
/**
 * Normalize a sort spec so the `vertical` and `horizontal` members are always arrays of per-
 * orientation specs, in priority order.  A single spec object (as used before multi-column sort, and
 * as saved in older perspectives) is wrapped in a one-element array.  The spec is normalized in
 * place and also returned.
 *
 * @param {View~SortSpec} sortSpec
 *
 * @returns {View~SortSpec} The same (now normalized) spec, or the input unchanged when it is null.
 */
 ...
 *
 * @property {Array.<View~SortSpecVert>} vertical
 * The vertical sort keys, in priority order: the first is the primary sort key, the second breaks
 * ties within equal primary values, and so on.  A single spec object is also accepted and is
 * normalized to a one-element array.
 *
 * @property {Array.<View~SortSpecHoriz>} horizontal
 * The horizontal sort keys, in priority order (see `vertical`).
 */
